...
- **Case-study protocol.** Four public codebases pinned as git submodules at exact tags, selected on coarse criteria only (domain diversity, direct JCA use) so selection cannot leak the fragility structure the score keys on. Apache Shiro was included as a **designed negative control** (expected near-zero asymmetric crypto). Mina SSHD was pinned deliberately *pre*-PQC (2.13.1): upstream added ML-KEM hybrids later, which preserves a clean Phase-2 mining candidate.
