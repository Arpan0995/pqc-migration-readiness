...
- Modes: `CLASSICAL`, `HYBRID`, `PQC_ONLY`, selected per *intent* (key-establishment vs signature), not globally; real migrations move the two at different speeds (HNDL makes KEM urgent first, see doc 01).
...
Model mirrors TLS named-groups / SSH kex lists, which are familiar and analyzable:
...
Negotiation scenarios (classical-only peer, hybrid peer, PQC-only peer, mismatch) are exactly the benchmark scenarios in doc 05, so one implementation serves both.
...
- Thread-safety: `getInstance` objects are not thread-safe, so the facade owns instance lifecycle (per-call or pooled; decide at implementation, benchmark both if cheap).
