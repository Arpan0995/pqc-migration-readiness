...
- **TP** = flagged site that genuinely requires change for a PQC migration. **FP** = flagged site needing no change (e.g., dead code path, test fixture, with the judgment logged). **FN** = labeled site the auditor missed (found by manual sweep of `java.security`/`javax.crypto`/BC imports).
...
| Outcome | Inference |
|---|---|
| ρ(S) high, Δρ > 0 | H1 supported: fragility signals predict cost beyond counting, so static triage is viable; the report's hotspot ranking is trustworthy |
...
**Headline number**: relative overhead (agility − direct)/direct per operation, i.e. the standard objection ("your abstraction is too slow") answered with data. Negotiation cost reported separately (expected ns–µs; it's a table intersection).
