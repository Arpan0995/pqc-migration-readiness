...
Status of facts verified: **2026-07-08** (web-checked; this is a fast-moving area, so re-verify before citing in a paper).
...
| JDK | PQC capability |
|---|---|
| 21 LTS (our target) | `javax.crypto.KEM` API exists; **no PQC algorithms**, so BC is required |
...
- The enterprise installed base (mostly 8/11/17/21) will not see native PQC for years, so an application-level agility layer on **JDK 21 + Bouncy Castle** targets exactly the population that needs migration tooling.
...
- A **negative result is publishable**: if fragility signals add nothing over raw counts, that tells organizations static triage is insufficient and dynamic/manual assessment is required, which is still actionable.
